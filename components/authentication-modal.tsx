...
export function AuthenticationModal({ onClose, canClose }: AuthenticationModalProps) {
 ...
 parsedError.data?.phone_number?.[0] ||
 parsedError.message ||
 err.message ||
 ...
 console.error("Error in handleSubmit:", JSON.stringify({
 message: err.message,
 data: err.response?.data,
 status: err.response?.status,
 }, null, 2), "Parsed Message:", errorMessage);
 setError(errorMessage);
 toast({
 title: "Login Failed",
 description: errorMessage,
 ...
 .then((verifyResponse) => {
 console.log("Final payment verification:", JSON.stringify(verifyResponse, null, 2));
 if (verifyResponse.status === "success" && verifyResponse.data.status === "completed") {
 setRequirePayment(false);
 toast({
 title: "Payment Successful",
